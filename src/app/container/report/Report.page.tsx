...
export default function RenderLibrary(props) {
    ...
    const downloadReceipt = (val) => {
        ...
        window.location.href = `${API_BASE}/public/money-receipt/download?invoiceIds=${val}&instituteId=${user?.instituteId}`
    }
                ...
                <>

                    {
                        mobileDisplay ?
                                        ...
                                        <List.Item key={item.key}>
                                            <Card className="mobileCard" onClick={() => { setIsModalVisible(true); setmodalItem(item) }}>
                                                <Row gutter={4}>
                                                    ...
                                                    <Col xs={24} sm={24} md={24} lg={3} xl={3}>
                                                        <span style={{ color: "blue", textDecoration: "underline" }}>#{item?.feeInvoiceId}</span>
                                                    </Col>
                                                </Row>
                            ...
                            <Table bordered={true} dataSource={reportData} columns={columns} className="custom-table" scroll={{ x: 1024 }} pagination={{ defaultPageSize: 10, showSizeChanger: true, pageSizeOptions: ['10', '20', '30'] }} />
